...
def run(args):
    ...
    # 创建alpha_schedule（如果需要）
    alpha_schedule = None
    if args.adaptive_levy and args.walk_type in ['adaptive_levy']:
        alpha_start = getattr(args, 'alpha_start', args.levy_alpha + 0.5)
        alpha_end = getattr(args, 'alpha_end', args.levy_alpha - 0.5)
        alpha_schedule = torch.linspace(alpha_start, alpha_end, args.length)
        alpha_schedule = torch.clamp(alpha_schedule, min=1.1, max=2.9)
        print(f"Created adaptive alpha schedule: {alpha_start:.2f} -> {alpha_end:.2f} over {args.length} steps")
    
    walker = Walker(data.edge_index, num_nodes=data.num_nodes, 
                   enable_levy=args.walk_type in ['levy', 'mixed', 'adaptive_levy'], 
    ...
    model = RUMModel(
        ...
        levy_ratio=args.levy_ratio,
        adaptive_levy=args.adaptive_levy,
...
if __name__ == '__main__':
    ...
    # Parameters related to Lévy's random wandering
    parser.add_argument('--walk_type', type=str, default='uniform', choices=['uniform', 'levy', 'mixed', 'adaptive_levy'], 
                       help='Type of random walk: uniform (standard), levy (Lévy walk), mixed (combination), or adaptive_levy (adaptive α)')
    parser.add_argument('--levy_alpha', type=float, default=1.5, 
                       help='Alpha parameter for Lévy distribution (0 < α ≤ 2), controls heavy-tail property')
    parser.add_argument('--levy_ratio', type=float, default=0.5, 
                       help='Ratio of Lévy walks in mixed mode (0.0 = all uniform, 1.0 = all Lévy)')
    parser.add_argument('--adaptive_levy', action='store_true', 
                       help='Enable adaptive α scheduling in Lévy walks')
    parser.add_argument('--alpha_start', type=float, default=2.0, 
                       help='Starting α value for adaptive Lévy walks')
    parser.add_argument('--alpha_end', type=float, default=1.2, 
                       help='Ending α value for adaptive Lévy walks')
