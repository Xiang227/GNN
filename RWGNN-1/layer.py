...
class RUMLayer(torch.nn.Module):
    def __init__(self, nin, nout, original_features,
        ...
        self.levy_ratio = levy_ratio
        self.adaptive_levy = adaptive_levy
        self.alpha_schedule = alpha_schedule
        
        if walk_type in ['levy', 'mixed', 'adaptive_levy'] and (walker is None or not walker.enable_levy):
            print(f"Warning: walk_type is '{walk_type}' but walker doesn't support Lévy walks. Falling back to uniform walks.")
            self.walk_type = 'uniform'

    def forward(self, x, edge_index, y0, e=None):
        if self.walk_type == 'levy' and self.walker.enable_levy:
            walks = self.walker.levy_random_walk(
                start_nodes=edge_index[0],
                walk_length=self.length,
                num_walks_per_node=self.num_samples,
            )
        elif self.walk_type == 'adaptive_levy' and self.walker.enable_levy:
            walks = self.walker.adaptive_levy_random_walk(
                start_nodes=edge_index[0],
                walk_length=self.length,
                num_walks_per_node=self.num_samples,
                alpha_schedule=self.alpha_schedule,
            )
        elif self.walk_type == 'mixed' and self.walker.enable_levy:
            walks = self.walker.mixed_random_walk(
                start_nodes=edge_index[0],
                walk_length=self.length,
                num_walks_per_node=self.num_samples,
                levy_ratio=self.levy_ratio,
            )
        else:
